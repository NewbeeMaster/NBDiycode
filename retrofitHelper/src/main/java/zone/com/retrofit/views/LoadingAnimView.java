...
public class LoadingAnimView extends View {
    ...
    private int hostRadius=-1;

    public int getHostRadius() {
        return hostRadius;
    }

    public void setHostRadius(int hostRadius) {
    ...
    /**
     * 理解：通过 实体类 的draw方法 来实现List的动画 逻辑划分的非常好！
     */
    class NodeList {
        private Paint paint;
    ...
    class Node {
        ...
        public void visit(float ft) {
            ...
            float hostRadiusTemp=getHostRadius()==-1?hostRadius:getHostRadius();
            cx = (float) Math.cos(angle) * hostRadiusTemp+ hostCX;
            cy = (float) Math.sin(angle) *hostRadiusTemp+ hostCY;
        }

        public void draw(Canvas canvas, Paint paint) {
            float radiusTemp=getHostRadius()==-1?radius:getRadius();
            canvas.drawCircle(cx, cy,radiusTemp, paint);
        }
    }
